...
- 请求发送后支持将响应内容导出为本地文件（自动推荐 json/xml/html/txt 后缀）
- 新增 `Send & Download`：发送成功后直接弹出保存对话框导出响应文件
...
#### 一键安装
可指定版本，下方示例中安装v0.0.6版本
```bash
curl -fsSL https://raw.githubusercontent.com/WangChengXiang-carizon/free-request/main/scripts/install.sh | bash -s -- v0.0.6
```
